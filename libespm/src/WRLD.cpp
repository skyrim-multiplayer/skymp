    ...
    [&](const char* type, uint32_t size, const char* data) {
      if (!std::memcmp(type, "FULL", 4)) {
        ...
        if (size == sizeof(WRLD::MapData::MapSize)) {
          result.mapData.mapSize = *reinterpret_cast<const WRLD::MapData::MapSize*>(data);
        } else {
        ...
        std::copy_n(data, std::size(result.bottomLeftCoord), result.bottomLeftCoord);
      } else if (!std::memcmp(type, "NAM9", 4)) {
        std::copy_n(data, std::size(result.topRightCoord), result.topRightCoord);
        ...
        std::copy_n(data, std::size(result.mapMarkerData), result.mapMarkerData);
