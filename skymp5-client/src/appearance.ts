import { Actor, ActorBase, Game, TESModPlatform, Race, HeadPart, TextureSet, printConsole, VoiceType, once, Utility } from 'skyrimPlatform'
...
export const getAppearance = (actor: Actor): Appearance => {
  ...
  const newAppearance: Appearance = {
    ...
    headpartIds: [],
    headTextureSetId: base.getFaceTextureSet() ? (base.getFaceTextureSet() as TextureSet).getFormID() : 0,
  ...
  const numTints = (Game.getPlayer() as Actor).getFormID() === actor.getFormID() ? Game.getNumTintMasks() : 0
...
export const applyTints = (actor: Actor | null, appearance: Appearance): void => {
  ...
  const raceSpecificWarPaint = tints.filter((t) => isVisible(t.argb) && t.texturePath.match(raceWarPaintRegex)).length // MaleHeadNordWarPaint
  const uniWarPaint = tints.filter((t) => isVisible(t.argb) && t.texturePath.match(uniWarPaintRegex)).length // MaleHeadWarPaint
...
const applyAppearanceCommon = (appearance: Appearance, npc: ActorBase): void => {
  ...
  const headparts = appearance.headpartIds.map((id) => HeadPart.from(Game.getFormEx(id))).filter((headpart) => !!headpart)
...
export const applyAppearanceToPlayer = (appearance: Appearance): void => {
  applyAppearanceCommon(appearance, ActorBase.from((Game.getPlayer() as Actor).getBaseObject()) as ActorBase)
