...
export const applyAnimation = (
  ...
  if (anim.animEventName === 'MRh_SpellRelease_Event') {
    const ac = Actor.from(refr);
    if (ac) {

      const rightSpell = ac.getEquippedSpell(SpellType.Right);
      if(rightSpell){
        printConsole(`rightSpell: ${rightSpell.getFormID().toString(16)} refr: ${ac.getFormID().toString(16)}`);
        castSpellImmediate(ac.getFormID(), SpellType.Right, rightSpell.getFormID() ,0);
      }
      ac.setAnimationVariableBool('bMRh_Ready', true);
      ac.setAnimationVariableBool('bMLh_Ready', true);

      ac.setAnimationVariableBool('bWantCastRight', true);
      ac.setAnimationVariableBool('IsCastingRight', true);

      ac.setAnimationVariableInt('iState_NPCMagic', 9);
      ac.setAnimationVariableInt('iState_NPCMagicCasting', 10);
    }
  } else if (anim.animEventName === 'MLH_SpellRelease_Event') {
    const ac = Actor.from(refr);
    if (ac) {

      const leftSpell = ac.getEquippedSpell(SpellType.Left);
      if(leftSpell){

        ac.setAnimationVariableBool('bMRh_Ready', true);
        ac.setAnimationVariableBool('bMLh_Ready', true);
        
        ac.setAnimationVariableBool('bWantCastLeft', true);
        ac.setAnimationVariableBool('IsCastingLeft', true);

        ac.setAnimationVariableInt('iState_NPCMagic', 9);
      ac.setAnimationVariableInt('iState_NPCMagicCasting', 10);

        printConsole(`leftSpell: ${leftSpell.getFormID().toString(16)} refr: ${ac.getFormID().toString(16)}`);
        castSpellImmediate(ac.getFormID(), SpellType.Left, leftSpell.getFormID() ,0);
      }
    }
  }
