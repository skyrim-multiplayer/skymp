...
export const removeAllSpells = (actor: Actor) => {
    ...
    for (let spell of spellToRemove) {
        ...
        printConsole(`removeResult: ${removeResult}, spellIdToRemove: ${spell.getFormID().toString(16)}, spellName: ${spell.getName()}`);
      }
}
...
export const learnSpells = (actor: Actor, spellsIds: Array<number>) => {
    ...
    for (let spellId of spellsIds) {
        ...
        if (spell) {
          ...
          printConsole(`addResult: ${addResult}, spellIdToLearn: ${spell.getFormID().toString(16)}, spellName: ${spell.getName()}`);
        }
      }
}
