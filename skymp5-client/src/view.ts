...
on('tick', () => {
  ...
  gUpdateNeighborFunctions = storage['updateNeighborFunctions'] as Record<string, any>
})
...
function isItem(t: number) {
  ...
  const isItem = isAmmo || isArmor || isBook || isIngredient || isLight || isPotion || isScroll || isSoulGem || isWeapon || isMisc
  return isItem
}
...
class SpawnProcess {
  constructor(appearance: Appearance, pos: NiPoint3, refrId: number, private callback: () => void) {
...
export class FormView implements View<FormModel> {
  ...
  update(model: FormModel): void {
    ...
    if (model.movement) {
      ...
      if (this.lastWorldOrCell !== model.movement.worldOrCell) {
        printConsole(
          `[1] worldOrCell changed, destroying FormView ${this.lastWorldOrCell.toString(16)} => ${model.movement.worldOrCell.toString(16)}`,
        )
    ...
    if (model.movement) {
      const worldOrCell = (Game.getPlayer() as Actor).getWorldSpace() || (Game.getPlayer() as Actor).getParentCell()
    ...
    if (refId) {
      ...
      const base = getFormEx(+(model.baseId as number)) || getFormEx(this.getAppearanceBasedBase())
      if (!base) return

      let refr = ObjectReference.from(Game.getFormEx(this.refrId))
      const respawnRequired = !refr || !refr.getBaseObject() || (refr.getBaseObject() as Form).getFormID() !== base.getFormID()
      ...
      if (respawnRequired) {
        ...
        if (base.getType() !== kTypeNpc) {
          refr.setAngle(model.movement?.rot[0] || 0, model.movement?.rot[1] || 0, model.movement?.rot[2] || 0)
        }
    ...
    if (base) {
      ...
      if (t >= 38 && t <= 39) {
        ...
        if (isHarvested != wasHarvested) {
          ...
          if (isHarvested)
            for (let i = 0; i < 20; ++i) {
              ac = Game.findRandomActor(refr.getPositionX(), refr.getPositionY(), refr.getPositionZ(), 10000) as Actor
  ...
  private applyAll(refr: ObjectReference, model: FormModel) {
    ...
    if (model.inventory && gCrosshairRefId == this.refrId && !isBadMenuShown()) {
    ...
    if (model.movement) {
      ...
      if (+(model.numMovementChanges as number) !== this.movState.lastNumChanges || Date.now() - this.movState.lastApply > 2000) {
    ...
    if (model.appearance) {
      ...
      if (actor && !gPcInJumpState) {
        ...
        const isOnScreen =
          screenPoint[0] > 0 && screenPoint[1] > 0 && screenPoint[2] > 0 && screenPoint[0] < 1 && screenPoint[1] < 1 && screenPoint[2] < 1
    ...
    if (this.refrId && model.appearance?.name) {
      ...
      const isVisibleByPlayer =
        !model.movement?.isSneaking && playerActor.getDistance(refr) <= maxNicknameDrawDistance && playerActor.hasLOS(refr)
...
class FormViewArray {
  ...
  updateAll(model: WorldModel, showMe: boolean, isCloneView: boolean) {
    ...
    for (let i = 0; i < n; ++i) {
      ...
      const offset = form.movement && (model.playerCharacterFormIdx === i || isCloneView)
      if (offset) {
        realPos = (form.movement as Movement).pos
        ;(form.movement as Movement).pos = [realPos[0] + 128, realPos[1] + 128, realPos[2]]
      }
      if (isCloneView) {
...
export class WorldView implements View<WorldModel> {
  ...
  update(model: WorldModel): void {
    ...
    const pcWorldOrCell = (Game.getPlayer() as Actor).getWorldSpace() || (Game.getPlayer() as Actor).getParentCell()
