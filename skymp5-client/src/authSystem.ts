...
const loginWithSkympIO = (
  data: LoginRegisterData,
  failCallback: (msg: string) => void,
  successCallback: (loginResponse: LoginResponseAuthData) => void,
): void => {
...
window.registerWidgetRegisterButton = {
  ...
  click: () => {
    const isPassEqual = window.registerData["${nameof<LoginRegisterData>(
      'password',
    )}"] === window.registerData["${nameof<LoginRegisterData>('passwordRepeat')}"];
    if (!isPassEqual) {
