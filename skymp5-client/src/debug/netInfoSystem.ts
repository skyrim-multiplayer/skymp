...
export const start = (): void => {
  ...
  sp.on('update', () => {
    ...
    sp.setTextString(
      textIds.connectionStateTextId,
      `${isConnected ? 'ON' : 'OFF'}`,
    );
    sp.setTextColor(
      textIds.connectionStateTextId,
      isConnected ? greenARGB : redARGB,
    );
    ...
    sp.setTextString(
      textIds.localPositionLagAmountTextId,
    ...
    sp.setTextString(
      textIds.receivedPacketAmountTextId,
      `${Math.round(NetInfo.getAndClearReceivedPacketCount())}`,
    );
    sp.setTextString(
      textIds.sentPacketAmountTextId,
      `${Math.round(NetInfo.getAndClearSentPacketCount())}`,
    );
    dt = 0;
  });
};
