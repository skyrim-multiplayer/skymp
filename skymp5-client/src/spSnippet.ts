...
const runMethod = async (snippet: Snippet): Promise<any> => {
  ...
  if (!self) throw new Error(`Unable to find form with id ${snippet.selfId.toString(16)}`)
  const selfCasted = spAny[snippet.class].from(self)
  if (!selfCasted) throw new Error(`Form ${snippet.selfId.toString(16)} is not instance of ${snippet.class}`)
...
const runStatic = async (snippet: Snippet): Promise<any> => {
  ...
  return await papyrusClass[snippet.function](...snippet.arguments.map((arg) => deserializeArg(arg)))
}
...
export const run = async (snippet: Snippet): Promise<any> => {
  if (snippet.class === 'SkympHacks') {
    if (snippet.function === 'AddItem' || snippet.function === 'RemoveItem') {
      ...
      if (count > 0) sp.Debug.notification(sign + ' ' + form.getName() + ' (' + count + ')')
