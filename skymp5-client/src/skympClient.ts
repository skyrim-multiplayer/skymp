  ...
  private sendAnimation(_refrId?: number) {
    ...
    if (!lastAnimationSent || anim.numChanges !== lastAnimationSent.numChanges) {
      if (anim.animEventName !== '') {
        ...
        if ((storage as Record<string, any>)._api_onAnimationEvent && (storage as Record<string, any>)._api_onAnimationEvent.callback) {
          try {
            ;(storage as Record<string, any>)._api_onAnimationEvent.callback(_refrId ? _refrId : 0x14, anim.animEventName)
          } catch (e) {
  ...
  private sendAppearance(_refrId?: number) {
    ...
    if (shown != this.isRaceSexMenuShown) {
      ...
      if (!shown) {
        ...
        this.sendTarget.send({ t: MsgType.UpdateAppearance, data: appearance, _refrId }, true)
      }
    }
  }
