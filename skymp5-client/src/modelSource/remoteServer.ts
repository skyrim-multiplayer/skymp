...
const onceLoad = (refrId: number, callback: (refr: ObjectReference) => void,
...
if (Array.isArray(storage["eventSourceContexts"])) {
  ...
  (storage["eventSourceContexts"] as any)
    .forEach((ctx: any) => { setupEventSource(ctx); });
}
...
on("tick", () => {
  ...
  if (loggingStartMoment &&
      Date.now() - loggingStartMoment > maxLoggingDelay) {
...
const loginWithSkympIoCredentials = () => {
  ...
  const authData =
    storage[AuthGameData.storageKey] as AuthGameData | undefined;
  if (authData?.local) {
    printConsole(
...
const setPcInventory = (inv: Inventory): void => { storage["pcInv"] = inv; };
...
on("update", () => {
  ...
  if (Date.now() - pcInvLastApply > 5000) {
    ...
    if (pcInv)
      applyInventory(Game.getPlayer() as Actor, pcInv, false, true);
  }
});
...
export class RemoteServer implements MsgHandler, ModelSource, SendTarget
{
  ...
  {
    once("update", () => {
      printConsole("Teleporting...", msg.pos, "cell/world is",
                   msg.worldOrCell.toString(16));
      // todo: think about track ragdoll state of player
      safeRemoveRagdollFromWorld(Game.getPlayer()!, () => {
        TESModPlatform.moveRefrToPosition(
          Game.getPlayer()!, Cell.from(Game.getFormEx(msg.worldOrCell)),
          WorldSpace.from(Game.getFormEx(msg.worldOrCell)), msg.pos[0],
          msg.pos[1], msg.pos[2], msg.rot[0], msg.rot[1], msg.rot[2]);
      });
    });
  }
  ...
  createActor(msg: messages.CreateActorMessage): void
  {
      ...
      onceLoad(refrId, (refr: ObjectReference) => {
        if (refr) {
          ...
          if (msg.props) {
            ...
            ModelApplyUtils.applyModelIsHarvested(refr,
    ...
    const applyPcInv = () => {
      applyInventory(Game.getPlayer() as Actor,
                     msg.equipment ? {
          ...
          (async () => {
            while (true) {
              ...
              TESModPlatform.moveRefrToPosition(
                ...
                msg.transform.pos[0], msg.transform.pos[1],
                msg.transform.pos[2], msg.transform.rot[0],
                msg.transform.rot[1], msg.transform.rot[2]);
              ...
              const distance = Math.sqrt(sqr(pos[0] - msg.transform.pos[0]) +
      ...
      once("tick", () => {
        once("tick", () => {
          if (!task.running) {
            ...
            printConsole("skinColorFromServer:",
                         msg.appearance ? msg.appearance.skinColor.toString(16)
                                        : undefined);
            loadGameManager.loadGame(
              msg.transform.pos, msg.transform.rot, msg.transform.worldOrCell,
              msg.appearance ? {
  ...
  DeathStateContainer(msg: messages.DeathStateContainerMessage): void
  {
    once("update",
         () => printConsole(
           `Received death state: ${JSON.stringify(msg.tIsDead)}`));
  ...
  setRaceMenuOpen(msg: messages.SetRaceMenuOpenMessage): void
  {
    if (msg.open) {
      ...
      once("update", () => Utility.wait(0.3).then(() => {
        unequipIronHelmet();
        Game.showRaceMenu();
      }));
    } else {
  ...
  private updateGamemodeUpdateFunctions(storageVar: string,
                                        functionSources:
    ...
    for (const propName of Object.keys(functionSources)) {
      try {
        (storage[storageVar] as any)[propName] =
          new Function("ctx", (storage[storageVar] as any)[propName]);
  ...
  updateGamemodeData(msg: messages.UpdateGamemodeDataMessage): void
  {
    ...
    this.updateGamemodeUpdateFunctions("updateNeighborFunctions",
                                       msg.updateNeighborFunctions || {});
    this.updateGamemodeUpdateFunctions("updateOwnerFunctions",
    ...
    if (!Array.isArray(storage["eventSourceContexts"])) {
      ...
      storage["eventSourceContexts"].forEach(
    ...
    eventNames.forEach((eventName) => {
      try {
        ...
        const ctx = {
          ...
          getFormIdInServerFormat :
            (clientsideFormId:
               number) => { return localIdToRemoteId(clientsideFormId); },
          getFormIdInClientFormat :
            (serversideFormId:
               number) => { return remoteIdToLocalId(serversideFormId); },
          _fn : fn,
  ...
  getMyActorIndex(): number { return this.worldModel.playerCharacterFormIdx; }
  ...
  send(msg: Record<string, unknown>, reliable: boolean): void
  {
    ...
    if (!this.worldModel.forms[idxInModel])
      return;
