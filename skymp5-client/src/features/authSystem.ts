...
const authUrl = normalizeUrl(
  (sp.settings['skymp5-client']['master'] as string) || 'https://skymp.io',
);
...
export const setPlayerAuthMode = (frozen: boolean): void => {
  if (frozen) {
    ...
    sp.Utility.setINIFloat(
      'fAutoVanityModeDelay:Camera',
      defaultAutoVanityModeDelay,
    );
  }
...
function createPlaySession(token: string) {
  ...
  if (!masterKey) {
    masterKey =
      sp.settings['skymp5-client']['server-ip'] +
      ':' +
      sp.settings['skymp5-client']['server-port'];
  }
  sp.printConsole({ masterKey });
  return client
    .post(`/api/users/me/play/${masterKey}`, {
      body: '{}',
  ...
  sp.once('browserMessage', (e) => {
    ...
    switch (eventKey) {
      case events.openDiscordOauth:
        sp.win32.loadUrl(
          `${authUrl}/api/users/login-discord?state=${discordAuthState}`,
        );
        break;
      case events.login:
        if (!authData) {
...
const loadLobby = (location: Transform): void => {
  sp.once('update', () => {
    defaultAutoVanityModeDelay = sp.Utility.getINIFloat(
      'fAutoVanityModeDelay:Camera',
    );
  ...
  loadGameManager.loadGame(location.pos, location.rot, location.worldOrCell);
};
...
const refreshWidgets = () => {
  ...
  sp.browser.executeJavaScript(
    new FunctionInfo(browsersideWidgetSetter).getText({
      events,
      browserState,
      authData,
    }),
