...
// This type is not fully decoded or correctly sized, just enough to use the
// functor
...
{
public:
  virtual ~IStackCallbackFunctor(){};

  virtual void Unk_01(BSScript::Variable var) = 0;
  virtual bool Unk_02(void) = 0;
  virtual void Unk_03(BSTSmartPointer<BSScript::Object>& arg1) = 0;
};

// 4B04
// this does more than hold on to class registrations, but for now that's all
// we care about
class VMClassRegistry
{
public:
  enum
  {
    kFunctionFlag_NoWait =
      0x01 // set this only if your function is thread-safe
  };
  ...
  virtual void PrintToDebugLog(const char* text, UInt32 stackId,
                               UInt32 logLevel);
  virtual void Unk_03(void);
  virtual void OnUpdate(float arg);
  virtual void OnUpdateGameTime(float arg);
  ...
  virtual bool CreateArray(VMValue* value, UInt32 size,
                           VMValue::ArrayData** unk1);
  virtual void Unk_17(void);
  virtual void RegisterFunction(IFunction* fn);
  virtual void SetFunctionFlagsEx(const char* className, UInt32 unk0,
                                  const char* fnName, UInt32 flags);
  virtual void SetFunctionFlags(const char* className, const char* fnName,
                                UInt32 flags);
  virtual void VisitScripts(UInt64 handle,
                            IForEachScriptObjectFunctor* functor);
  virtual bool Unk_1C(UInt64 handle, const char* className,
  ...
  virtual bool ExtractValue(
    UInt64 handle, StringCache::Ref* className, SInt32 variableIndex,
    VMValue* out); // unk probably script context object?
  virtual void QueueEvent(UInt64 handle, const StringCache::Ref* eventName,
                          IFunctionArguments* args);
  virtual void Unk_25(void);
  virtual void CallStaticFunction(
    const BSFixedString& className, const BSFixedString& fnName,
    IFunctionArguments* args, BSTSmartPointer<IStackCallbackFunctor>& result);
  virtual void CallMemberFunction(
    BSTSmartPointer<BSScript::Object>& obj, const BSFixedString& fnName,
    IFunctionArguments* args, BSTSmartPointer<IStackCallbackFunctor>& result);
  virtual void CallMemberFunctionFromHandle(
    UInt64 handle, const BSFixedString& className, const BSFixedString& fnName,
    IFunctionArguments* args, BSTSmartPointer<IStackCallbackFunctor>& result);
  ...
  void LogError(const char* message, UInt32 stackId)
  {
    PrintToDebugLog(message, stackId, kLogLevel_Error);
  }
  void LogWarning(const char* message, UInt32 stackId)
  {
    PrintToDebugLog(message, stackId, kLogLevel_Warning);
  }
};
...
public:
  ...
  DEFINE_MEMBER_FN(UnregisterFromSleep_Internal, void, 0x009247E0,
                   UInt64 handle);
  DEFINE_MEMBER_FN(RevertGlobalData_Internal, bool, 0x0092DAE0);
  // E79F47684196468F785E6950402DBDAA5A668FAA+110
  DEFINE_MEMBER_FN(SaveRegSleepEventHandles_Internal, bool, 0x0092EB50,
                   void* handleReaderWriter, void* saveStorageWrapper);
  // ECCDB3740458A9BB07FA958498C66F04CE14F94C+1E6
  DEFINE_MEMBER_FN(LoadRegSleepEventHandles_Internal, bool, 0x0092FD80,
                   void* handleReaderWriter, void* loadStorageWrapper);

  // 00011FEFBFDA69F253C84165B45736A4F0AF02B8+204
  DEFINE_MEMBER_FN(QueueDelayFunctor_Internal, bool, 0x009252C0,
                   void** pFunctor);
