...
interface NetworkingClient {
  create(host: string, port: number): void
  destroy(): void
  isConnected(): boolean
  handlePackets(handler: (packetType: PacketType, jsonContent: string, error: string) => void): void
  send(jsonContent: string, reliable: boolean): void
}
export declare let networkingClient: NetworkingClient
