...
RE::BSEventNotifyControl GameEventSinks::ProcessEvent(
  const RE::ItemCrafted::Event* event,
  RE::BSTEventSource<RE::ItemCrafted::Event>* eventSource)
{
  if (event == nullptr) {
    return RE::BSEventNotifyControl::kContinue;
  }

  auto form = event->item;
  auto formId = form ? form->formID : 0;
  auto weaponAlchemyFlag = event->unk08;
  auto enchantingFlag = event->unk09;

  SkyrimPlatform::GetSingleton().AddUpdateTask(
    [form, formId, weaponAlchemyFlag, enchantingFlag] {
      auto obj = JsValue::Object();

      auto formLocal = RE::TESForm::LookupByID(formId);

      if (formLocal == nullptr || formLocal != form) {
        return;
      }

      obj.SetProperty("item", CreateObject("Form", formLocal));

      if (weaponAlchemyFlag) {
        EventsApi::SendEvent("itemCrafted", { JsValue::Undefined(), obj });
      }
      if (enchantingFlag) {
        EventsApi::SendEvent("itemEnchanted", { JsValue::Undefined(), obj });
      }
    });
...
RE::BSEventNotifyControl GameEventSinks::ProcessEvent(
  const RE::Inventory::Event* event,
  RE::BSTEventSource<RE::Inventory::Event>* eventSource)
{
  if (event == nullptr) {
    return RE::BSEventNotifyControl::kContinue;
  }

  auto refr = event->objRefr;
  auto refrId = refr ? refr->formID : 0;
  auto entryData = event->entryData;
  auto newCount = event->newCount;
  auto prevCount = event->prevCount;
  auto itemName = entryData->GetDisplayName();
  auto itemOwner = entryData->GetOwner();
  auto itemOwnerId = itemOwner ? itemOwner->formID : 0;
  auto itemValue = entryData->GetValue();
  auto itemWeight = entryData->GetWeight();
  auto itemSoulLevel = static_cast<int>(entryData->GetSoulLevel());

  if (itemName == nullptr || itemValue < 0 || itemWeight < 0 ||
      itemSoulLevel < 0 || itemSoulLevel > 5 || newCount < 0 ||
      prevCount < 0) {
    return RE::BSEventNotifyControl::kContinue;
  }

  SkyrimPlatform::GetSingleton().AddUpdateTask(
    [refr, refrId, newCount, prevCount, itemName, itemOwner, itemOwnerId,
     itemValue, itemWeight, itemSoulLevel] {
      auto obj = JsValue::Object();

      auto refrLocal = RE::TESForm::LookupByID(refrId);
      auto itemOwnerLocal = RE::TESForm::LookupByID(itemOwnerId);

      if (refrLocal == nullptr || refrLocal != refr) {
        return;
      }

      obj.SetProperty("reference", CreateObject("ObjectReference", refrLocal));
      obj.SetProperty("itemName", JsValue::String(itemName));
      if (itemOwnerLocal != nullptr || itemOwner == itemOwnerLocal) {
        obj.SetProperty("itemOwner",
                        CreateObject("ObjectReference", itemOwnerLocal));
      }
      obj.SetProperty("itemValue", JsValue::Double(itemValue));
      obj.SetProperty("itemWeight", JsValue::Double(itemWeight));
      obj.SetProperty("itemSoulLevel", JsValue::Double(itemSoulLevel));
...
RE::BSEventNotifyControl GameEventSinks::ProcessEvent(
  const RE::ChestsLooted::Event* event,
  RE::BSTEventSource<RE::ChestsLooted::Event>* eventSource)
{
  if (event == nullptr) {
    return RE::BSEventNotifyControl::kContinue;
  }
  ...
  SkyrimPlatform::GetSingleton().AddUpdateTask([] {
    ...
    EventsApi::SendEvent("chestLooted", { JsValue::Undefined(), obj });
  });
...
RE::BSEventNotifyControl GameEventSinks::ProcessEvent(
  ...
  RE::BSTEventSource<RE::ItemsPickpocketed::Event>* eventSource)
{
  if (event == nullptr) {
    return RE::BSEventNotifyControl::kContinue;
  }

  auto itemCount = event->numItems;

  SkyrimPlatform::GetSingleton().AddUpdateTask([itemCount] {
    auto obj = JsValue::Object();
    ...
    obj.SetProperty("count", JsValue::Double(itemCount));

    EventsApi::SendEvent("itemPickpocketed", { JsValue::Undefined(), obj });
  });
