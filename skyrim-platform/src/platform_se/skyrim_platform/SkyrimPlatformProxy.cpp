    ...
    getNativeCallRequirements)
{
  ...
  handler.Set(
    "get",
    ProxyGetter([=](const Napi::Object& origin, const Napi::String& keyStr) {
      auto env = origin.Env();
      ...
      if (!fRef) {
        ...
        if (isGame && keyStrExtracted == "getFormEx") {
            ...
            auto f = Napi::Function::New(
              ...
              NapiHelper::WrapCppExceptions(
                [](const Napi::CallbackInfo& info) -> Napi::Value {
                  CallNative::ObjectPtr obj =
                    NativeValueCasts::JsObjectToNativeObject(
                      NapiHelper::ExtractObject(info[0], "objActorCandidate"));
                  if (!obj)
                    return info.Env().Null();
                  auto form = (RE::TESForm*)obj->GetNativeObjectPtr();
                  if (!form)
          ...
          auto f = Napi::Function::New(
            env,
            NapiHelper::WrapCppExceptions(
              [keyStrExtracted, className, getNativeCallRequirements](
                const Napi::CallbackInfo& info) -> Napi::Value {
  ...
  Napi::Object proxyObject =
    proxyConstructor.Call({ skyrimPlatformExports, handler });
  return proxyObject;
}
}
    ...
    getNativeCallRequirements)
{
  ...
  handler.Set(
    "get",
    ProxyGetter([skyrimPlatformExportsRef](const Napi::Object& origin,
                                           const Napi
                                           : String& keyStr) {
      ...
      if (!proxyRef) {
        auto proxy =
          GetProxyForClass(keyStrExtracted, skyrimPlatformExportsRef->Value(),
                           getNativeCallRequirements);
        proxyRef.reset(
          new Napi::Reference<Napi::Object>(Napi::Persistent(proxy)));
      }
      return proxyRef;
    }));
