...
struct FunctionHook
{
  ...
  explicit FunctionHook(void** appSystemFunction,
...
public:
  ...
  void Add(T** aSystemFunction, U* aHookFunction,
           bool aDelayed = false) noexcept
  {
    Add(FunctionHook(reinterpret_cast<void**>(aSystemFunction),
                     reinterpret_cast<void*>(aHookFunction)),
        aDelayed);
  }

  template <class T>
  void* AddSystem(const std::string& acLibraryName,
                  const std::string& acFunctionName, T* apFunction) noexcept
  {
    const auto module = LoadLibraryA(acLibraryName.c_str());
    if (module != nullptr) {
      auto pSystemFunction = reinterpret_cast<void*>(
        GetProcAddress(module, acFunctionName.c_str()));
...
#define TP_HOOK(systemFunction, hookFunction)                                 \
  CEFUtils::FunctionHookManager::GetInstance().Add(systemFunction,            \
                                                   hookFunction, true)
#define TP_HOOK_IMMEDIATE(systemFunction, hookFunction)                       \
  CEFUtils::FunctionHookManager::GetInstance().Add(systemFunction,            \
                                                   hookFunction, false)
