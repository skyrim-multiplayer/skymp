...
FunctionHook::FunctionHook(void** appSystemFunction,
                           void* apHookFunction) noexcept
...
void FunctionHookManager::InstallDelayedHooks() noexcept
{
  ...
  const auto pHooks = static_cast<HOOK_INFO*>(
    allocator.Allocate(sizeof(HOOK_INFO) * m_delayedHooks.size()));
...
{
  StackAllocator<1 << 12> allocator;
  const auto pHooks = static_cast<void***>(
    allocator.Allocate(sizeof(void**) * m_installedHooks.size()));
...
void FunctionHookManager::Add(FunctionHook aFunctionHook,
                              const bool aDelayed) noexcept
{
  if (aDelayed)
    m_delayedHooks.emplace_back(std::move(aFunctionHook));
  else if (Mhook_SetHook(aFunctionHook.m_ppSystemFunction,
                         aFunctionHook.m_pHookFunction) == TRUE)
                                  ...
                                  const char* acpMethod) noexcept
{
  ...
  auto pImageNtHeaders = reinterpret_cast<PIMAGE_NT_HEADERS>(
    RtlOffsetToPointer(pBase, pImageDosHeader->e_lfanew));

  const auto pVirtualAddress =
    pImageNtHeaders->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT]
      .VirtualAddress;

  for (auto pImageImportDescriptor =
         reinterpret_cast<PIMAGE_IMPORT_DESCRIPTOR>(
           RtlOffsetToPointer(pBase, pVirtualAddress));
       pImageImportDescriptor->Name; ++pImageImportDescriptor) {
    const auto pLibraryName = reinterpret_cast<const char*> RtlOffsetToPointer(
      pBase, pImageImportDescriptor->Name);

    if (!_stricmp(pLibraryName, acpLibraryName)) {
      auto pImportAddressTable = reinterpret_cast<uintptr_t*>(
        RtlOffsetToPointer(pBase, pImageImportDescriptor->FirstThunk));
      auto pImageThunkData = reinterpret_cast<IMAGE_THUNK_DATA*>(
        RtlOffsetToPointer(pBase, pImageImportDescriptor->OriginalFirstThunk));

      while (const auto pOrdinal = pImageThunkData->u1.Ordinal) {
        const auto pFunctionName =
          reinterpret_cast<PIMAGE_IMPORT_BY_NAME>(
            RtlOffsetToPointer(pBase, pImageThunkData->u1.AddressOfData))
            ->Name;
