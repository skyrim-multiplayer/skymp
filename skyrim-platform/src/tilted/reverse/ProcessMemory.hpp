...
struct ProcessMemory
{
  ...
  {
    return WriteBuffer(reinterpret_cast<const unsigned char*>(&acData),
                       sizeof(T), aOffset);
  }

  bool WriteBuffer(const unsigned char* acpData, size_t aSize,
                   size_t aOffset) const noexcept;
...
bool Write(const TAddress acAddress, const TVar acVal,
           const size_t acOffset = 0)
{
...
bool Nop(const TAddress acAddress, size_t aLength)
{
  ...
  return memory.WriteBuffer(
    reinterpret_cast<const unsigned char*>(ptr.GetPtr()), aLength, 0);
}
}
