  ...
  , m_textSize(0)
{
  ...
  const auto ntHeaders = reinterpret_cast<PIMAGE_NT_HEADERS>(
    ...
    static_cast<uintptr_t>(
      reinterpret_cast<PIMAGE_DOS_HEADER>(m_baseAddress)->e_lfanew));
  ...
  m_textHash = FHash::Crc64(
    reinterpret_cast<const unsigned char*>(m_textStartAddress), m_textSize);
}
...
void* AutoPtrManager::Find(Pattern aPattern) const noexcept
{
  ...
  size_t i = 0;
  while (true) {
    const auto res = std::search(cstart, cend, aPattern.BytePattern.begin(),
                                 aPattern.BytePattern.end(), cmpOp);
    if (res >= cend)
      break;
    ...
    if (aPattern.Index == i) {
      if (aPattern.Type == Pattern::kRelativeIndirection4) {
        ...
        return (void*)(address + *reinterpret_cast<const int32_t*>(address) +
                       4);
      }
