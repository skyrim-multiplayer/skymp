...
using TReset = HRESULT(WINAPI*)(
  IDirect3DDevice9* pDevice, D3DPRESENT_PARAMETERS* pPresentationParameters);
using TPresent = HRESULT(WINAPI*)(IDirect3DDevice9* pDevice,
                                  const RECT* pSourceRect,
                                  const RECT* pDestRect,
                                  HWND hDestWindowOverride,
                                  const RGNDATA* pDirtyRegion);
using TCreateDevice = HRESULT(WINAPI*)(
  IDirect3D9* apDirect3D9, UINT Adapter, D3DDEVTYPE DeviceType,
...
static HRESULT __stdcall HookReset(
  IDirect3DDevice9* pDevice, D3DPRESENT_PARAMETERS* pPresentationParameters)
{
...
static HRESULT __stdcall HookPresent(IDirect3DDevice9* pDevice,
                                     ...
                                     RGNDATA* pDirtyRegion)
{
  ...
  const auto result = RealPresent(pDevice, pSourceRect, pDestRect,
                                  hDestWindowOverride, pDirtyRegion);

  return result;
}

HRESULT __stdcall HookCreateDevice(
  IDirect3D9* apDirect3D9, UINT Adapter, D3DDEVTYPE DeviceType,
  HWND hFocusWindow, DWORD BehaviorFlags,
  D3DPRESENT_PARAMETERS* pPresentationParameters,
  IDirect3DDevice9** ppReturnedDeviceInterface)
{
  ...
  const auto result = RealCreateDevice(
    apDirect3D9, Adapter, DeviceType, hFocusWindow, BehaviorFlags,
...
void D3D9Hook::Install() noexcept
{
  if (RealDirect3DCreate9 == nullptr)
    RealDirect3DCreate9 = reinterpret_cast<TDirect3DCreate9>(
      TP_HOOK_SYSTEM("d3d9.dll", "Direct3DCreate9", HookDirect3DCreate9));
}
