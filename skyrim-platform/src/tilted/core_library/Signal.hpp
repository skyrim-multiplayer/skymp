...
{
  bool Invoke(Collector& collector, const std::function<R(Args...)>& cbf,
              Args... args) const
  {
...
{
  bool Invoke(Collector& collector, const std::function<void(Args...)>& cbf,
              Args... args) const
  {
    cbf(args...);
    return collector();
...
private:
  ...
  bool Remove(size_t id)
  {
    auto it = std::remove_if(
      begin(callback_list_), end(callback_list_),
...
public:
  ...
  size_t Connect(const CbFunction& aCallbackFunction)
  {
    return Add(aCallbackFunction);
  }
  ...
  [[nodiscard]] std::size_t Count() const { return callback_list_.size(); }
};

}

template <typename SignalSignature,
          class Collector = details::CollectorDefault<
