...
using Map = std::unordered_map<T, U, std::hash<T>, std::equal_to<T>,
                               StlAllocator<std::pair<const T, U>>>;

template <class T>
using Set =
  std::unordered_set<T, std::hash<T>, std::equal_to<T>, StlAllocator<T>>;
...
using WString =
  std::basic_string<wchar_t, std::char_traits<wchar_t>, StlAllocator<wchar_t>>;
...
{
  return std::allocate_shared<T>(StlAllocator<T>(),
