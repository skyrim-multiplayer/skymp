...
int main(int argc, char* argv[])
{
    ...
    auto init = (int (*)(int, char**))GetProcAddress(handle, "SkyrimNodeJS_Init");
    auto createEnvironment = (int (*)(int, char**, void**))GetProcAddress(handle, "SkyrimNodeJS_CreateEnvironment");
    auto destroyEnvironment = (int (*)(void*))GetProcAddress(handle, "SkyrimNodeJS_DestroyEnvironment");
    auto tick = (int (*)(void*))GetProcAddress(handle, "SkyrimNodeJS_Tick");
    auto executeScript = (int (*)(void*, const char*))GetProcAddress(handle, "SkyrimNodeJS_ExecuteScript");
    auto getError = (uint64_t (*)(char*, uint64_t))GetProcAddress(handle, "SkyrimNodeJS_GetError");

    if (init == NULL || createEnvironment == NULL || destroyEnvironment == NULL || tick == NULL || executeScript == NULL || getError == NULL)
    {
