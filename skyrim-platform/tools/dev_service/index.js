    ...
    if (fileName === 'touch_Release' || fileName === 'touch_Debug') {
      ...
      if (process.platform === 'win32') {
        ...
        ;[
          ...
          cp(path.join(cefDir, 'Release', item), path.join(distDir, 'Data/Platform/Distribution/RuntimeDependencies'))
        })
        ...
        ;['icudtl.dat'].forEach((item, i) => {
          cp(path.join(cefDir, 'Resources', item), path.join(distDir, 'Data/Platform/Distribution/RuntimeDependencies'))
        })
        ;['cef.pak', 'cef_100_percent.pak', 'cef_200_percent.pak', 'cef_extensions.pak', 'devtools_resources.pak'].forEach((item, i) => {
          cp(path.join(cefDir, 'Resources', item), path.join(distDir, 'Data/Platform/Distribution/CEF'))
        })
        fs.copySync(path.join(cefDir, 'Resources/locales'), path.join(distDir, 'Data/Platform/Distribution/CEF/locales'))

        cp(binPath('SkyrimPlatform.pdb'), distDir)
        cp(binPath('SkyrimPlatformImpl.pdb'), distDir)
        cp(binPath('ChakraCore.dll'), path.join(distDir, 'Data/Platform/Distribution/RuntimeDependencies'))
        cp(binPath('SkyrimPlatformCEF.exe.hidden'), path.join(distDir, 'Data/Platform/Distribution/RuntimeDependencies'))
        cp(binPath('SkyrimPlatformCEF.pdb'), distDir)
        cp(binPath('SkyrimPlatform.dll'), path.join(distDir, 'Data/SKSE/Plugins'))
        cp(binPath('SkyrimPlatformImpl.dll'), path.join(distDir, 'Data/Platform/Distribution/RuntimeDependencies'))
        cp(path.join(sourceDir, `src/platform_se/pex/TESModPlatform.pex`), path.join(distDir, 'Data/Scripts'))
        cp(`${getBinaryDir()}/skymp5-server/cpp/${buildCfg}/MpClientPlugin.dll`, path.join(distDir, 'Data/SKSE/Plugins'))
        cp(path.join(sourceDir, 'tools/system_polyfill/dist/___systemPolyfill.js'), path.join(distDir, 'Data/Platform/Distribution'))
        fs.copySync(path.join(sourceDir, 'tools/plugin-example'), path.join(distDir, 'Data/Platform/plugin-example'))
      ...
      cp(path.join(bin, `_codegen/skyrimPlatform.ts`), path.join(distDir, 'Data/Platform/Modules'))
      cp(path.join(bin, `_codegen/skyrimPlatform.ts`), path.join(sourceDir, 'src/platform_se/codegen/convert-files'))
