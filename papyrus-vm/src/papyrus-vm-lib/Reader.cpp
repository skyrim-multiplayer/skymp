...
Reader::Reader(const std::vector<std::string>& pexPaths)
{
  for (auto& path : pexPaths) {
    CreateScriptStructure(ReadFileBinary(path));
  }
}

Reader::Reader(const std::vector<std::vector<uint8_t>>& pexData)
{
  for (auto& pex : pexData) {
...
{
  SLNet::BitStream bitStream(const_cast<uint8_t*>(arrayBytes.data()),
                             arrayBytes.size(), false);

  constexpr int kBytesToIgnore = []() -> int {
    int result = 0;
  ...
  bitStream.IgnoreBytes(kBytesToIgnore);

  std::shared_ptr<PexScript> structure =
    std::make_shared<PexScript>(stringTable, arrayBytes);
}

std::vector<uint8_t> Reader::ReadFileBinary(const std::string& path)
{
...
//   Fdebug.stateName =
//   this->structure->stringTable.GetStorage()[Read16_bit()]; Fdebug.fnName =
//   this->structure->stringTable.GetStorage()[Read16_bit()]; Fdebug.type =
//   Read8_bit();
...
//     this->structure->stringTable.GetStorage()[Read16_bit()];
//   object.docstring =
//   this->structure->stringTable.GetStorage()[Read16_bit()]; object.userFlags
//   = Read32_bit(); object.autoStateName =
//     this->structure->stringTable.GetStorage()[Read16_bit()];
