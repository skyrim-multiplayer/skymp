...
Napi::Value ScampServer::LookupEspmRecordById(const Napi::CallbackInfo& info)
{
  try {
    ...
    if (lookupRes.rec) {
      ...
      espmLookupResult.Set("toGlobalRecordId", Napi::Function::New(info.Env(), [lookupRes](const Napi::CallbackInfo &info) {
        auto localRecordId = NapiHelper::ExtractUInt32(info[0], "localRecordId");
