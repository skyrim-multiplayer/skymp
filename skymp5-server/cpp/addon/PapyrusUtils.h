...
static Napi::Value GetJsObjectFromPapyrusObject(
  Napi::Env env, const VarValue& value, const std::vector<std::string>& espmFilenames)
{
...
static Napi::Value GetJsValueFromPapyrusValue(Napi::Env env, const VarValue& value, const std::vector<std::string>& espmFilenames)
{
  if (value.promise) {
    auto promiseCallback = Napi::Function::New(env,
      [value, espmFilenames](const Napi::CallbackInfo& info) {
        auto resolve = info[0].As<Napi::Function>();
        std::shared_ptr<Napi::Reference<Napi::Function>> resolveRef(new Napi::Reference<Napi::Function>(Napi::Persistent<Napi::Function>(resolve)));

        value.promise->Then([env = info.Env(), resolveRef, espmFilenames](const VarValue& v) {
            std::vector<Napi::Value> resolveArgs = { GetJsValueFromPapyrusValue(env, v, espmFilenames) };
            resolveRef->Value().Call(resolveArgs);
        });

        auto reject = info[1].As<Napi::Function>();
        std::shared_ptr<Napi::Reference<Napi::Function>> rejectRef(new Napi::Reference<Napi::Function>(Napi::Persistent<Napi::Function>(reject)));

        value.promise->Catch([env = info.Env(), rejectRef, espmFilenames](const char *what) {
            std::vector<Napi::Value> rejectArgs = { Napi::String::New(env, what) };
  ...
  switch (value.GetType()) {
    ...
    case VarValue::kType_BoolArray: {
      ...
      for (uint32_t i = 0; i < n; ++i) {
        arr.Set(i, GetJsValueFromPapyrusValue(env, (*value.pArray)[i], espmFilenames));
      }
      return arr;
    }
  }
...
static VarValue GetPapyrusValueFromJsValue(const Napi::Value& v, bool treatNumberAsInt,
                                    WorldState& wst)
{
    switch(v.Type())
    {
        ...
        case napi_number:
            return treatNumberAsInt ? VarValue(v.As<Napi::Number>().Int32Value()) : VarValue(v.As<Napi::Number>().DoubleValue());
        case napi_string:
        ...
        case napi_object:
        {
            ...
            if (obj.IsArray())
            {
                ...
                for (uint32_t i = 0; i < n; ++i) {
                    arrayContents->push_back(GetPapyrusValueFromJsValue(arr.Get(i), treatNumberAsInt, wst));
                    auto extractedType = arrayContents->back().GetType();
                    if (type == static_cast<uint8_t>(~0)) {
                        type = extractedType;
                    } else if (extractedType != type) {
                        throw std::runtime_error("Papyrus doesn't support heterogeneous arrays");
                    }
                }
            ...
            {
                // TODO: consider removing promise support. wouldn't be better if we always wait promises on js side instead of passing to papyrus?
                auto obj = v.As<Napi::Object>();
                if (v.IsPromise()) {
                    VarValue res = VarValue::None();
                    res.promise = std::make_shared<Viet::Promise<VarValue>>();

                    auto thenCallback = Napi::Function::New(v.Env(), [res, &wst](const Napi::CallbackInfo &info) {
                        // TODO: should we always set treatNumberAsInt to false?
                        bool treatNumberAsInt = false;
                        res.promise->Resolve(GetPapyrusValueFromJsValue(info[0], treatNumberAsInt, wst));
                    });
        ...
        default:
            throw std::runtime_error("Unknown JavaScript type " + std::to_string(v.Type()));
    }
}
