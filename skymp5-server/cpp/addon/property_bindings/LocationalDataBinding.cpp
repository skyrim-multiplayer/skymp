...
void LocationalDataBinding::Set(Napi::Env, ScampServer &scampServer, uint32_t formId, Napi::Value newValue) {
  auto &partOne = scampServer.GetPartOne();
  ...
  auto newLocationalData = NapiHelper::ExtractObject(newValue, "newLocationalData");
  locationalData.cellOrWorldDesc = FormDesc::FromString(NapiHelper::ExtractString(newLocationalData.Get("cellOrWorldDesc"), "newLocationalData.cellOrWorldDesc"));
  locationalData.pos = NapiHelper::ExtractNiPoint3(newLocationalData.Get("pos"), "newLocationalData.pos");
  locationalData.rot = NapiHelper::ExtractNiPoint3(newLocationalData.Get("rot"), "newLocationalData.rot");
...
void LocationalDataBinding::Apply(MpActor &actor, const LocationalData &locationalData) {
