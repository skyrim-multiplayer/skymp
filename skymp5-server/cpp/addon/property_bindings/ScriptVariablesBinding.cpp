...
namespace {
    template <class F>
    void ForEachScriptVariable(const std::shared_ptr<IGameObject> &gameObject, const F &callback) {
...
Napi::Value ScriptVariablesBinding::Get(Napi::Env env, ScampServer& scampServer, uint32_t formId) {
  ...
  ForEachScriptVariable(gameObject, [&](const std::string &scriptName, const CIMap<VarValue> &variables) {
  ...
  ForEachScriptVariable(gameObject, [&](const std::string &scriptName, const CIMap<VarValue> &variables) {
    for (auto pair : variables) {
        ...
        element.Set("value", PapyrusUtils::GetJsValueFromPapyrusValue(env, pair.second, partOne->worldState.espmFiles));
