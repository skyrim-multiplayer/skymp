...
namespace {
    Napi::Value ToNapiValue(const PacketHistory &history, const PacketHistoryElement &element, Napi::Env env) {
        if (element.offset >= history.buffer.size()) {
            throw std::runtime_error("PacketHistoryWrapper: element.offset >= history.buffer.size()");
        }
        if (element.offset + element.length > history.buffer.size()) {
            throw std::runtime_error("PacketHistoryWrapper: element.offset + element.length > history.buffer.size()");
        }
...
PacketHistory PacketHistoryWrapper::FromNapiValue(const Napi::Object &packetHistory) {
    ...
    for (uint32_t i = 0; i < packets.Length(); ++i) {
      ...
      std::string tip1 =
        "packetHistory.packets." + std::to_string(i) + ".data";
      std::string tip2 =
        "packetHistory.packets." + std::to_string(i) + ".timeMs";

      auto packet = NapiHelper::ExtractObject(packets.Get(i), tip.data());

      auto buffer = NapiHelper::ExtractUInt8Array(packet.Get("data"), tip1.data());
...
Napi::Object PacketHistoryWrapper::ToNapiValue(const PacketHistory &history, Napi::Env env) {
    ...
    for (int i = 0; i < static_cast<int>(history.packets.size()); ++i) {
      ...
      element.Set("data",
                  ::ToNapiValue(history, history.packets[i], env));
      element.Set("timeMs",
                  Napi::Number::New(env, history.packets[i].timeMs));
      arr.Set(i, element);
    }
