...
const { master, port, maxPlayers, name, ip, gamemodePath, offlineMode } = Settings.get()
...
const handleLibkeyJs = () => {
  ...
  setTimeout(async () => {
    while (1) {
      ...
      if (data !== libkey.src) {
        await new Promise<void>((r) => fs.writeFile('data/_libkey.js', libkey.src, () => r()))
      }
    }
  }, 1)
}
...
const main = async () => {
  ...
  if (!fs.existsSync(absoluteGamemodePath)) {
    log(`Error during loading a gamemode from "${absoluteGamemodePath}" - file or directory does not exist`)
  } else {
    try {
