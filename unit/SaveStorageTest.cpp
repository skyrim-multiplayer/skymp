...
#include "database_drivers/ArchiveDatabase.h"
...
std::shared_ptr<ISaveStorage> MakeSaveStorageArchive()
{
  ...
  return std::make_shared<AsyncSaveStorage>(
    std::make_shared<ArchiveDatabase>(archivePath, spdlog::default_logger()),
    spdlog::default_logger(), "archive");
}

std::vector<std::shared_ptr<ISaveStorage>> MakeSaveStorages()
{
  return { MakeSaveStorageFile(), MakeSaveStorageArchive() };
}
