...
TEST_CASE("SimdJsonArchive variant - extended types and edge cases", "[Archives] [Serialization]")
{
  // Extended variant with more types (removed std::nullptr_t)
  using ExtendedJsonTestParam = std::variant<bool, int8_t, uint8_t, int16_t, uint16_t, int32_t, uint32_t, int64_t, uint64_t, float, double, std::string>;
  ExtendedJsonTestParam param;

  // Test all types
  param = true;
  REQUIRE(std::get<bool>(ParseWithSimdInputArchive<ExtendedJsonTestParam>("true")) == true);
  param = 123;
  ...
  REQUIRE(foundFloat);
  param = "test";
  REQUIRE(std::get<std::string>(ParseWithSimdInputArchive<ExtendedJsonTestParam>("\"test\"")) == "test");

  // Nested variant
  using NestedVariant = std::variant<int, std::variant<std::string, double>>;
  nlohmann::json j = std::string("nested");
  auto nested = ParseWithSimdInputArchive<NestedVariant>(nlohmann::to_string(j));
  REQUIRE(std::holds_alternative<std::variant<std::string, double>>(nested));
  REQUIRE(std::get<std::string>(std::get<std::variant<std::string, double>>(nested)) == "nested");

  j = 2.71;
  nested = ParseWithSimdInputArchive<NestedVariant>(nlohmann::to_string(j));
  REQUIRE(std::holds_alternative<std::variant<std::string, double>>(nested));
  REQUIRE(std::get<double>(std::get<std::variant<std::string, double>>(nested)) == 2.71);
}
