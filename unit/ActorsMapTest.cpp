...
{
  ActorsMap map;
  auto actor = std::make_shared<MpActor>(LocationalData(), FormCallbacks(), std::nullopt);
  map.Set(0, actor.get());
  map.Set(1, actor.get());
...
{
  ActorsMap map;
  auto actorA = std::make_shared<MpActor>(LocationalData(), FormCallbacks(), std::nullopt);
  auto actorB = std::make_shared<MpActor>(LocationalData(), FormCallbacks(), std::nullopt);
