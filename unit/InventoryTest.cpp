...
TEST_CASE("RemoveItems in case of multiple entries with the same baseId/extras", "[Inventory]") {
    Inventory inv;
...
TEST_CASE("Similar entries should be merged if needed to remove items", "[Inventory]") {
    Inventory inv;
    inv.entries.push_back({ 0xf, 10, {} });
    inv.entries.push_back({ 0xf, 10, {} });
