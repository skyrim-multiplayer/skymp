...
const Chat = (props) => {
  ...
  const getMessageSpans = (text, currentColor = undefined) => {
    ...
    for (let i = 0; i + colorSnippetTpl.length < text.length; ++i) {
      if (text[i] == '#' && text[i + 1] == '{' && text[i + colorSnippetTpl.length - 1] == '}') {
        return (
          <span style={{ color: currentColor }}>
            {text.substring(0, i)}
            {getMessageSpans(text.substring(i + colorSnippetTpl.length), '#' + text.substring(i + 2, i + 8))}
          </span>
        )
      }
    }
    return <span style={{ color: currentColor }}>{text}</span>
  }

  const getList = () => {
    return props.messages.map((msg, index) => (
      <div className="msg" key={`msg-${index}`} style={{ marginLeft: '10px' }}>
        {getMessageSpans(msg)}
      </div>
    ))
  }
