...
const Constructor = (props) => {
      ...
      switch (props.elem.type) {
        case 'form':
          const isContentInitialized =
            content_mainRef && content_mainRef.current && content_mainRef.current.clientHeight && content_mainRef.current.clientWidth
  ...
  switch (rend.type) {
        ...
        line.forEach((obj, elementIndex) => {
          ...
          if (curElem !== undefined) {
            arr.push(
              hasHint ? (
                <div key={key}>
                  <SkyrimHint text={hints[hintIndex].text} isOpened={hints[hintIndex].isOpened} left={hints[hintIndex].direction} />
                  <div onMouseOver={() => setHintState(obj.index, true)} onMouseOut={() => setHintState(obj.index, false)}>
                    {curElem}
                  </div>
                </div>
              ) : (
        ...
        <div className={'login'}>
          <div className={'login-form'} style={{ width: `${fwidth}px`, height: `${fheight}px` }}>
            <div className={'login-form--content'}>
              {result.header !== undefined ? <div className={'login-form--content_header'}>{result.header}</div> : ''}
    ...
    case 'chat':
      return <Chat messages={rend.messages} send={rend.send} placeholder={rend.placeholder} isInputHidden={rend.isInputHidden} />
    default:
      break
  }
}
