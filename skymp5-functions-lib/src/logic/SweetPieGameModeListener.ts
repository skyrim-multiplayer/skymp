...
import { ChatMessage, ChatProperty, ChatText, createSystemMessage} from "../props/chatProperty";
...
export class SweetPieGameModeListener implements GameModeListener {
  ...
  onPlayerChatInput(actorId: number, input: string, neighbors: number[], masterApiId: number) {      
    if (input.startsWith('/')) {
      ...
      const argsRaw = args.slice(1).join(' ');
      
      if (command === "record") {
        if (args[1] === 'start') {
          this.controller.sendChatMessage(actorId, ChatMessage.system('Start'));
          this.controller.setPacketHistoryRecording(actorId, true);
        }
        else if (args[1] === 'stop') {
          this.controller.sendChatMessage(actorId, ChatMessage.system('Stop'));
          this.controller.setPacketHistoryRecording(actorId, false);
        }
        else if (args[1] === 'clear') {
          this.controller.sendChatMessage(actorId, ChatMessage.system('Clear'));
          this.controller.clearPacketHistory(actorId);
        }
        else if (args[1] === 'play') {
          const history = this.controller.getPacketHistory(actorId);

          if (history.packets.length === 0) {
            this.controller.sendChatMessage(actorId, ChatMessage.system("Nothing to play"));
          }
          else {
            // /record start
            // /record stop
            // /record play
            // /record clear
            const playbackMs = history.packets[0].timeMs - history.packets[history.packets.length - 1].timeMs;

            this.controller.sendChatMessage(actorId, ChatMessage.system(`Play ${history.packets.length} packets, ${playbackMs} ms`));
            this.controller.requestPacketHistoryPlayback(actorId, history);
          }
        }
        else {
          // send tip
          this.controller.sendChatMessage(actorId, ChatMessage.system('Usage: /record [start|stop|play]'));
        }
      }
      return;
    }
