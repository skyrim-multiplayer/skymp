    ...
    if (kitName === 'base') {
      ...
      if (baseKitDiscordRole === undefined) {
        ...
        this.controller.sendChatMessage(
          actorId,
          ChatMessage.system('discordAuth.baseKitRoleId should be set in server-settings.json', this.controller)
        );
        ...
        if (!previousDate || currentDate - previousDate > secondsBetweenGifts * 1000) {
          this.controller.sendChatMessage(
            actorId,
            ChatMessage.system('You have been given a base kit', this.controller)
          );
          ...
          this.controller.sendChatMessage(
            actorId,
            ChatMessage.system(`${diffSeconds} seconds remaining`, this.controller)
          );
        }
      } else {
        this.controller.sendChatMessage(
          actorId,
          ChatMessage.system('You do not have permission to use this kit', this.controller)
        );
      }
    }
