  ...
  handle(input: HandlerInput): void {
    ...
    if (role === undefined) {
      ...
      this.controller.sendChatMessage(
        actorId,
        ChatMessage.system('discordAuth.adminRoleId should be set in server-settings.json', this.controller)
      );
      ...
      for (const targetPlayerActorId of this.controller.getOnlinePlayers()) {
        if (this.controller.getProfileId(targetPlayerActorId) === targetMasterApiId) {
          this.controller.sendChatMessage(
            actorId,
            createSystemMessage(`Kicked actor ${targetPlayerActorId.toString(16)}`)
          );
          this.controller.sendChatMessage(
            targetPlayerActorId,
            createSystemMessage('You have been kicked. Exiting game in 5 seconds.')
          );
          setTimeout(() => {
      ...
      this.controller.sendChatMessage(
        actorId,
        ChatMessage.system('You do not have permission to use this command', this.controller)
      );
    }
  }
