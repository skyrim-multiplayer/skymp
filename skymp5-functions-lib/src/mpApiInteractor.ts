...
const scriptName =
  (refrId: number) => {
    ...
    if (lookupRes.record) {
      const vmadIndex =
        lookupRes.record.fields.findIndex((field) => field.type === 'VMAD');
      if (vmadIndex >= 0) {
...
const isTeleportDoor = (refrId: number) => {
  ...
  if (lookupRes.record) {
    const xtelIndex =
      lookupRes.record.fields.findIndex((field) => field.type === 'XTEL');
    return xtelIndex !== -1;
  }
  return false;
};
...
export class MpApiInteractor
{
  ...
  private static setupActivateHandler(listener: GameModeListener)
  {
    mp.onActivate = (target: number, caster: number) => {
      ...
      const baseRecType =
        mp.lookupEspmRecordById(mp.getIdFromDesc(baseDesc)).record?.type;
      const targetDesc = mp.getDescFromId(target);

      const res = listener.onPlayerActivateObject!
                  (caster, targetDesc, target, baseRecType!);
      if (res)
        return true;
  ...
  private static setupChatHandler(listener: GameModeListener)
  {
    ChatProperty.setChatInputHandler((input) => {
      const chatSettings =
        this.serverSettings.chatSettings as ChatSettings ?? {};
      ...
      const actorNeighbors =
        mp.get(input.actorId, 'actorNeighbors')
          ...
          .filter((actorId) =>
                    ...
                    getActorDistanceSquared(input.actorId, actorId) <
                      sqr(chatSettings.hearingRadiusNormal));

      const name = getName(input.actorId);
      if (listener.onPlayerChatInput) {
        console.log(`chat: ${JSON.stringify(name)} (${
          input.actorId.toString(16)}): ${JSON.stringify(input.inputText)}`);
        listener.onPlayerChatInput(input.actorId, input.inputText,
                                   actorNeighbors, name);
      }
    });
  }
  ...
  private static setupDialogResponseHandler(listener: GameModeListener)
  {
    DialogProperty.setDialogResponseHandler((response) => {
      if (listener.onPlayerDialogResponse) {
        listener.onPlayerDialogResponse(response.actorId, response.dialogId,
  ...
  private static setupTimer(listener: GameModeListener)
  {
    Timer.everySecond = () => {
      ...
      const joinedPlayers =
        onlinePlayers.filter((x) => !onlinePlayersOld.includes(x));
      const leftPlayers =
        onlinePlayersOld.filter((x) => !onlinePlayers.includes(x));
      ...
      for (const actorId of onlinePlayers) {
        ...
        EvalProperty.eval(actorId, () => {
          for (const [formId, name] of nameUpdatesClientSide) {
            const refr =
              ctx.sp.ObjectReference.from(ctx.sp.Game.getFormEx(formId));
  ...
  static makeController(): PlayerController
  {
    return
    {
      showMessageBox(actorId: number, dialogId: number, caption: string,
                     text: string, buttons: string[]): void
      {
        DialogProperty.showMessageBox(actorId, dialogId, caption.toLowerCase(),
                                      text.toLowerCase(),
      ...
      {
        EvalProperty.eval(actorId, () => { ctx.sp.win32.exitProcess(); });
      }
      ...
      , addItem(actorId: number, itemId: number, count: number): void
      {
        mp.callPapyrusFunction(
          ...
          { type : 'form', desc : mp.getDescFromId(actorId) }, [
            { type : 'espm', desc : mp.getDescFromId(itemId) }, count,
            /*silent*/ false
          ]);
      }
