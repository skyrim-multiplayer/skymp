...
export class CarryAnimSystem {
  ...
  private static clientsideInitEvent() {
    return () => {
      ...
      if (
        typeof ctx.sp.storage.sweetCarryEquippedFormId !== 'number' &&
      ...
      const SWEET_CARRY_ANIM_RESTRICT = ['Jump*', 'SprintStart', 'WeapEquip'];
      ...
      ctx.sp.on('combatState', (event) => {
        if (
          event.isCombat &&
          event.actor.getFormID() == playerId &&
          ctx.sp.storage.sweetCarryAnimationActive == true &&
          ctx.sp.storage.sweetCarryEquippedFormId
        ) {
