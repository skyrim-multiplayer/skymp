...
void AddInnerTracerMessages(const std::vector<InnerExecutionEvent>& evts, ResolvedContextEntry& rent) {
  ...
  auto& m = rent.messages.emplace_back();
  m.type = "inner_tracer";
  m.strval = std::move(ss).str();
}
} // namespace
  ...
  ANTIGO_TRY {
    ...
    if (h.slow != nullptr) {
      ...
      if (h.downCtx == nullptr || h.downCtx->h.slow == nullptr || h.downCtx->h.slow->innerExecEvts == nullptr) {
        auto w = ResolveCtxStackImpl("inner_tracer");
        if (w.entries.empty()) {
          auto& e = w.entries.emplace_back();
          e.sourceLoc.filename = "n/a";
          e.sourceLoc.line = 0;
        ...
        // evts->push_back({h.filename, h.linenum, h.funcname, "leave", ResolveCurrentImpl()});
